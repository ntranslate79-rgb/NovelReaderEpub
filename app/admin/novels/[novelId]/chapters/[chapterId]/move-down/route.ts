...
export async function POST(
  ...
  if (!validateCsrfToken(csrfToken)) {
    ...
    }).catch((e) => console.error("Failed to log move-down audit:", e));
  ...
  try {
    ...
    await logAuditAction({
      action: "CHAPTER_MOVE",
      resource: `Chapter:${cId}`,
      details: { novelId: nId, fromNumber: chapter.number, toNumber: next.number },
      success: true,
    }).catch((e) => console.error("Failed to log move-down audit:", e));
  } catch (error) {
    await logAuditAction({
    ...
    }).catch((e) => console.error("Failed to log move-down error audit:", e));
  }
